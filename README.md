...
- **多主题系统**：支持 `科技深色 / 纸墨浅色 / 霓虹紫蓝 / 森林绿金` 全局切换，并持久化保存。
- **性能优化**：
...
- 设置页支持四套固定主题切换，切换后全局页面即时生效。
- 主题偏好持久化到 DataStore，重启应用后保持上次选择。
- 已完成主要业务页 `MaterialTheme.colorScheme` 全覆盖与视觉对比微调。
...
## 13. 平台功能四（非遗交互体验）开发计划

围绕“智能编曲 + AI 导师点评”两条主链路，按“先可演示、再可增强、后可运营”推进：

### 13.1 目标

- 打通输入 -> 生成/点评 -> 反馈 -> 沉淀的完整闭环。
- 建立可量化指标：生成、点评、试听、收藏、最近事件等。
- 形成可复用的演示能力，便于后续接入真实模型服务。

### 13.2 分阶段安排（6 周）

- **Week 1-2（MVP）**
  - 智能编曲：参数输入、结果生成、试听/收藏、基础埋点。
  - AI 点评：音频输入（演示态）、多维评分、建议输出、基础埋点。
- **Week 3-4（体验增强）**
  - 编曲多方案输出、风格模板库、快速生成模式（1 分钟）。
  - 点评维度细化（节奏/音准/表现力），生成阶段性练习建议。
- **Week 5-6（运营化）**
  - 历史/收藏/最近事件沉淀与成长视图。
  - 演示报告导出、版权说明与成果留痕。
...
- 两条主链路稳定可演示，操作路径完整。
- 交互数据可追踪，统计结果可刷新与回看。
- 构建校验通过：`.\gradlew :app:compileDebugKotlin`。
...
- 先重新安装最新 `debug` 包，避免旧缓存样式残留。
- 在设置页切换主题后返回主链路页面逐页检查（音乐馆/故事/社区/商城/我的）。
- 若新增页面，请优先使用 `MaterialTheme.colorScheme.*`，避免写死颜色值。
