# -*- coding: utf-8 -*-
"""从 `data/故事/*.txt` 生成：
  - `miniprogram/data/story.ts`（主包列表，无正文）、`miniprogram/pkg/data/storyBodies.ts`（分包正文）；
  - `web/src/data/story.ts`、`StoriesData.kt`。
...
OUT_MP_BODIES = ROOT / "miniprogram" / "pkg" / "data" / "storyBodies.ts"
...
def emit_mp_lite(entries: list[dict], path: Path):
    """小程序主包：仅列表元数据，满足主包 ≤1.5M 代码质量检查。"""
    lines: list[str] = []
    lines.append("/**")
    lines.append(" * AUTO-GENERATED by `tools/sync_stories_data.py` from `data/故事/*.txt`.")
    lines.append(" * 主包仅含列表字段（无正文）。完整正文见 `pkg/data/storyBodies.ts`。")
    lines.append(" */")
    lines.append('import { assetUrl } from "../utils/assetUrl";')
    lines.append("")
    lines.append("export interface Story {")
    lines.append("  id: string;")
    lines.append("  title: string;")
    ...
    lines.append("}")
    lines.append("")
    lines.append('export type StoryCard = Omit<Story, "body">;')
    lines.append("")
    cats = ", ".join(f'"{c}"' for c in STORY_CATEGORIES)
    lines.append(f"export const STORY_CATEGORIES = [{cats}] as const;")
    lines.append("")
    lines.append("export const STORIES: StoryCard[] = [")
    for e in entries:
        tags_js = ", ".join(json.dumps(t, ensure_ascii=False) for t in e["tags"])
        lines.append("  {")
        lines.append(f'    id: "{e["id"]}",')
        ...
        lines.append(f"    readCount: {e['readCount']}, ")
        lines.append(f"    tags: [{tags_js}], ")
        lines.append(f'    coverSrc: assetUrl("/images/story/{e["key"]}.jpg"),')
        lines.append(f"    coverWidth: {e['coverWidth']},")
        lines.append(f"    coverHeight: {e['coverHeight']},")
        lines.append("  },")
    lines.append("];")
    lines.append("")
    lines.append("export function storyById(id: string): StoryCard | undefined {")
    lines.append("  return STORIES.find((s) => s.id === id);")
    lines.append("}")
    lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")
...
def emit_mp_bodies(entries: list[dict], path: Path):
    """小程序分包：完整正文，仅详情页引用。"""
    lines: list[str] = []
    lines.append("/**")
    lines.append(" * AUTO-GENERATED by `tools/sync_stories_data.py` — 完整正文，勿在主包 import。")
    lines.append(" */")
    lines.append('import type { Story } from "../../data/story";')
    lines.append('import { assetUrl } from "../../utils/assetUrl";')
    lines.append("")
    lines.append("export const STORIES_FULL: Story[] = [")
    for e in entries:
        tags_js = ", ".join(json.dumps(t, ensure_ascii=False) for t in e["tags"])
        lines.append("  {")
        lines.append(f'    id: "{e["id"]}",')
    ...
    lines.append("];")
    lines.append("")
    lines.append("export function storyByIdFull(id: string): Story | undefined {")
    lines.append("  return STORIES_FULL.find((s) => s.id === id);")
    lines.append("}")
    lines.append("")
...
def main():
    ...
    emit_mp_lite(entries, OUT_MP)
    emit_mp_bodies(entries, OUT_MP_BODIES)
    ...
    print(
        ...
        "stories ->",
        OUT_MP.name,
        OUT_MP_BODIES.relative_to(ROOT),
        OUT_WEB.relative_to(ROOT),
        OUT_KT.relative_to(ROOT),
    )
    return 0
