...
Page({
  ...
  goChangePwd() { wx.navigateTo({ url: "/pkg/change-password/index" }); },
  goAgreement() { wx.navigateTo({ url: "/pkg/webview/index?title=用户协议" }); },
  goPrivacy() { wx.navigateTo({ url: "/pkg/webview/index?title=隐私政策" }); },
  goAbout() { wx.navigateTo({ url: "/pkg/about/index" }); },
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          wx.reLaunch({ url: "/pkg/login/index" });
        }
      },
    });
  },
